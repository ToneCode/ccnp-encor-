...
* build a list of candidate WLCs to join via
  ...
  * Broadcast on the local subnet to solicit controllers
    * _CAPWAP Discovery Request_
      * broadcast to its subnet broadcast addr. (*.255), or
...
* if AP and WLC on different subnets
  * a router can help to relay the broadcast requests on UDP port 5246 to WLC IP addresses
...
1. If the AP has previously joined a controller or it knows its primary, secondary, and tertiary controller
...
3. join the least-loaded WLC
   ...
   * each controller reports its load during discovery
...
* config AP
  ...
  * Things that define layer 1 like the RF operation on each wireless band
...
* a key derivation and distribution protocol
* after the client is successfully authenticated, a Pairwise Master Key (PMK) and a Groupwise Master Key (GMK) that get generated and distributed during the EAP authentication.
...
* _ANonce_: a random number generated by the access point
  * a unique nonce for client to use in key derivation
